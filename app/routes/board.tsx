...
function CardItem({
  ...
  // The board is where the person came from, so the link carries it and the
  ...
  return (
    ...
    >
      <span className="flex items-baseline gap-2">
        ...
        <Link to={taskPath(slug, card.id, here)} className="flex-1 underline-offset-2 hover:underline">
          {card.title}
        </Link>
