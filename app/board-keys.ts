 ...
 * share are the same letters by decision, not by one shared list. The letters
 * themselves come from `app/key-map.ts`, which is where a key lives whoever
 * binds it. See ADR-0016.
 */
...
export function boardPress(key: string, columns: KeyedColumn[], on: string | null): Act | null {
  ...
  if (fires("next", key)) return pick(at === -1 ? rows[0] : rows[Math.min(at + 1, rows.length - 1)]);
  if (fires("prev", key)) return pick(at === -1 ? rows[rows.length - 1] : rows[Math.max(at - 1, 0)]);
  ...
  if (fires("clear", key)) return on === null ? null : { act: "on", id: null };
  ...
  if (fires("open", key)) return { act: "open", id };
  ...
  if (fires("forward", key) || fires("back", key)) {
    const status = stepped(column.status, fires("forward", key) ? 1 : -1);
  ...
  if (fires("down", key) || fires("up", key)) {
    const at = column.ids.indexOf(id);
    if (fires("up", key)) return at === 0 ? null : { act: "step", id, way: "up" };
...
/**
 * Binds the keys to the columns the board draws, and answers with the props
 * each column's list of cards takes.
 *
 * The cursor names a card, not a place, so a card the person moves keeps the
 * cursor while the board redraws around it.
 *
 * The keys are live while focus is inside one of those lists and nowhere else,
 * and the arrows cross the columns the letters walk. See ADR-0022.
 */
export function useBoardKeys(
  ...
  return useKeyedList((key) => {
    ...
    const act = boardPress(key, columns, on);
    if (!act) return false;
