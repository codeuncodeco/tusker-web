...
on the org board — the column appears on its own when To do and In progress are
both empty — and by request everywhere else.

Cancelled is where work ended too, but it is off the run: a card reaches it by a
drag or the select and never by a step. See [ADR-0015](./0015-a-drop-names-a-column-not-a-place.md). It is the outcome a person
looks up, not the one they work from, so it stays a request.
